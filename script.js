...
async function cargarProductos() {
    ...
    try {
        ...
        // Validar la estructura de la respuesta y los campos relevantes
        console.log('Respuesta remota (raw):', data);
        const productosRemotos = Array.isArray(data.productos) ? data.productos : [];
        const productosValidos = productosRemotos.filter(p => p && typeof p.nombre === 'string' && typeof p.precio === 'number' && !Number.isNaN(p.precio));

        // Si la respuesta es exitosa y contiene productos válidos, los usamos
        if (data && data.status === 'success' && productosValidos.length > 0) {
            productosJugos = productosValidos;
            console.log('✅ Productos cargados desde Google Sheets:', productosJugos.length);
        } else {
            // Si no hay productos válidos, usamos el fallback
            console.warn('⚠️ Respuesta remota inválida o sin productos válidos. Usando lista local.');
            usarFallback = true;
        }
    } catch (error) {
